...
**최종 업데이트:** 2026-03-10 (서버: exp010 V3 소규모 비교, exp011 V3 전체 파이프라인 완료, 세션 6 추가)
**관리 규칙:** 매 세션 종료 시 섹션 10(세션 로그)에 변경사항 추가. 새 실험 결과는 섹션 5에 추가. 새 논문 발견 시 섹션 3에 추가.
**코딩 규칙** 매 실험에 대해 재현 가능하도록 해당 실험의 이름(예: exp001)을 붙이고, 이를 위한 config 파일을 따로 작성. 해당 config 파일에는 실험 수행을 위한 command도 함께 작성. 본인이 맡은 영역(예: 우리 방법론 실험, 재현 검증)에 해당하는 폴더 내에서만 코드 수정. 그 외 폴더의 코드는 관리의 일관성을 위해 수정 불가. 수정 요청을 해야 할 경우 반드시 명확한 이유와 함께 보고.
...
### 5.8 Track B V3 임베딩 (exp010) — 소규모 비교 (2026-03-09)
- **변경점**: V1(CLS token, top layer, 평균풀링) → V3(chat template, last-token, layer[-2])
- **평가 기준**: Heterogeneity (코사인 유사도), Extreme Contrastive L2

| 지표 | V1 (기준선) | V3 |
|------|------------|-----|
| Persona het mean | 0.9960 | **0.9728** |
| Persona het std | 0.0017 | **0.0221** |
| Persona het >0.99 비율 | 99.4% | **38.4%** |
| Item het mean | 0.9989 | **0.9834** |
| Item het >0.99 비율 | 100.0% | **62.7%** |
| Extreme Contrastive L2 | 기준 | **10~60배 증가** |

- **결론**: V3 방향 유효 — embedding collapse 개선, 구분력 확인
- config: `configs/exp010_v3_comparison.yaml` | script: `scripts/exp010_v3_comparison.py`

### 5.9 Track B V3 전체 파이프라인 (exp011) — 2026-03-10 완료 (총 ~20시간)
- **설정**: warm 15,000 texts / cold 5,000 texts, V3 임베딩, 동일 3개 head
- **V1(exp009) vs V3(exp011) 비교**:

| Head | V1 cold MAE | V3 cold MAE (raw) | V3 α | V3 α-MAE | V1 DirAcc | V3 DirAcc |
|------|------------|-------------------|------|----------|----------|----------|
| Ridge (mean) | 68.30 | 73.32 | 0.147 | 11.38 | 0.536 | 0.525 |
| Attn + Linear | 67.94 | 68.14 | 0.135 | 9.66 | 0.537 | 0.524 |
| **Attn + Bottleneck** | 59.45 | 67.71 | 0.135 | 10.29 | 0.536 | **0.550** |

- **PCA 분석** (V3 warm embeddings):
  - top-10 PC: **99.59%** (V1: 98.6%) → 분산이 더 집중됨
  - 98.6% 달성에 필요한 PC 수: **5개** (V1 대비 더 저랭크)
  - top-1 PC가 57.9% 설명 → embedding collapse 여전히 존재
- **해석**:
  - raw MAE: V3 ≈ V1 (동등하거나 소폭 악화) — 상대 비교만 유효, 절대 평가는 Phase 2 이후
  - DirAcc: Attn+Bottleneck만 V3가 소폭 개선(0.536 → 0.550), 나머지 유사
  - α 보정 MAE 10 내외는 α ~0.13~0.15에 의한 scale 축소 효과
  - **exp006 weekly MAE(8~9)와 단위 불일치 문제 미해결** → MAE 스케일 확인 필요
- config: `configs/exp011_v3_pipeline.yaml` | script: `scripts/run_exp011.py`
- 결과 경로: `experiments/exp011_v3_pipeline/results/`
...
### 즉시 필요
- [ ] **MAE 스케일 불일치 확인**: exp011 MAE(60~73) vs exp006 weekly MAE(8~9) — 단위/스케일 원인 규명 (exp009부터 지속 미해결)
- [ ] exp011 Attention weights 분석 (어떤 페르소나에 높은 가중치?)
- [ ] exp006과 exp011의 통합 비교 테이블 (동일 단위 확보 후)
- [ ] embedding collapse 근본 원인 분석 — V3에서도 top-1 PC가 57.9% 설명 → 프롬프트/레이어 추가 탐색 필요
...
### 세션 6 (2026-03-09~10, 서버): Track B V3 임베딩 비교 및 전체 파이프라인
**담당:** 서버 (Track B 방법론 실험)
**주요 작업 및 결과:**

1. **exp010: V3 임베딩 소규모 비교 (2026-03-09)**
   - V3 = chat template + last-token + layer[-2]
   - Persona heterogeneity >0.99 비율: V1(99.4%) → V3(38.4%), Extreme Contrastive L2 10~60배 증가
   - V3 방향 유효 확인 → 전체 파이프라인 적용 결정
   - config: `configs/exp010_v3_comparison.yaml`

2. **exp011: V3 전체 파이프라인 (2026-03-09 시작 → 2026-03-10 02:00 완료, ~20시간)**
   - warm 15,000 texts → cold 5,000 texts → 3개 head 학습 → PCA
   - Ridge mean: cold MAE 73.32, DirAcc 0.5249
   - Attn + Linear: cold MAE 68.14, DirAcc 0.5241
   - Attn + Bottleneck: cold MAE 67.71, DirAcc 0.5502 (DirAcc 최고)
   - PCA: V3 top-10 PC = 99.59%, 98.6% 달성에 5개 PC만 필요 (V1 대비 더 저랭크)
   - config: `configs/exp011_v3_pipeline.yaml`

3. **미해결 이슈**
   - MAE 스케일 불일치(exp011 MAE 60~73 vs exp006 weekly 8~9) 원인 미규명
   - embedding collapse(top-1 PC 57.9%) 여전히 존재 — V3로도 해소 안 됨
